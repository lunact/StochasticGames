...
def maximin(game, expected_rewards, state):
    ...
    try:
        ...
        pi_s = np.array([mod.addVar(vtype=GRB.CONTINUOUS, name="p_"+str(action0)) for action0 in game.actions(0, state)])
        mod.update()
        ...
        # Constraints
        for action1 in game.actions(1, state):
            # conversion dictionary > np.array while preserving the same order for actions
            expected_rewards_action1 = np.array([expected_rewards[action1][action0] for action0 in game.actions(0, state)])
