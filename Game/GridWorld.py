...
class GridWorld(StochasticGame):
    """
    Example of a null sum 2-player stochastic game: GridWorld Game 2 (Hu and Wellman 2003)
    """
    ...
    def rewards(self, state, actions): # immediate rewards
        pos1, pos2 = state
        dest1 = self.next_position(pos1, actions[0])
        dest2 = self.next_position(pos2, actions[1])

        #goal
        if dest1 == self.goal_position and dest2 == self.goal_position :
            return {0:100, 1:100}
        if dest1 == self.goal_position : #player 1 goal
            return {0:100, 1:0}
        if dest2 == self.goal_position : #player 2 goal
            return {0:0, 1:100}
        #run into each other
        if dest1 == dest2 : 
            return {0:-1, 1:-1}
