...
def testWoLF(shapleyepsilon,iterationlittman,iterationwolf,nbplay):
    #setup (+random policy)
    print("\ngame RPS")
    game = RockPaperScissors()
    p0, p1 = game.players()[0], game.players()[1]
    randompolicy = {state: {action: 1/len(game.actions(p1, state)) for action in game.actions(p1, state)} for state in game.states()}

    #run shapley, et train littman, wolf contre randompolicy
    print("run shapley  epsilon = ",shapleyepsilon)
    start_time = time.time()
    pi_shap,V_shap = shapley(game, epsilon = shapleyepsilon,playerA = p1,playerB = p0)
    print("Time shapley : ",time.time() - start_time)
    ...
    print("run Littman with  ",iterationlittman," iteration" )
    start_time = time.time()
    pi_l , V_l = MinimaxQ(game,explor = .3,decay = .01 ** (1. / ( 2 *10**4)),display = 10000,iteration = iterationlittman, playerA = p0,playerB = p1 ,policyplayerB = randompolicy)
    print("Time Littman : ",time.time() - start_time)

    print("run WoLF-PHC with  ",iterationwolf," iteration" )
    start_time = time.time()
    pi_wolf = WoLF_PHC(game, explor = .3, decay = .01 ** (1. / ( 2 *10**4)), delta_win = 0.0025, delta_lose = 0.01, display = 10000, iteration = iterationwolf, playerA = p0, playerB = p1, policyplayerB = randompolicy)
    print("Time WoLF : ",time.time() - start_time)


    #play (random vs wolf), (shapley vs wolf), (littman vs wolf)
    print("Test ",nbplay," game")
    policy  = {p0 : pi_l , p1 : pi_wolf }
    test = Test(game,nbplay,policy )
    test.simulation()
    print("total_rewards")
    print(test.total_rewards)
...
#testWoLF(shapleyepsilon = 0.0,iterationlittman = 2 *  10 ** 4, iterationwolf = 2 * 10 ** 4,nbplay = 10**4)

def test_wolf_RPS(iterationwolf):
    print("\ngame RPS")
    game = RockPaperScissors()
    p0, p1 = game.players()[0], game.players()[1]
    randompolicy = {state: {action: 1/len(game.actions(p1, state)) for action in game.actions(p1, state)} for state in game.states()}
...
#test_wolf_RPS(100000)


def test_wolf_soccer(iterationwolf):
    print("\ngame RPS")
    game = Soccer()
    p0, p1 = game.players()[0], game.players()[1]
    randompolicy = {state: {action: 1/len(game.actions(p1, state)) for action in game.actions(p1, state)} for state in game.states()}
