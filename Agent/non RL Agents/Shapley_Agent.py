...
class Shapley_Agent(non_RL_Agent):
    """
    A Specific Agent class for Shapley Agent
    """
    ...
    def new_V_and_pi(self,V):
        ...
        G = {state: \
                 {o: \
                      {a : \
                           G_a(state, {self.playerID : a, (1-self.playerID): o}, V) \
                      for a in self.g.actions(self.playerID, state)} \
                 for o in self.g.actions((1-self.playerID), state)} \
            for state in self.g.states()}
