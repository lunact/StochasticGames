...
class PD_WoLF_Agent(RL_Agent):
    ...
    def learn(self,s,s2,k,a,o,rewA,rewB,opponent_policy):
        ...
        # 2) determine if winning or losing and choice of delta
        if self.Delta[s][a]*self.Delta2[s][a] < 0:
            self.delta = self.delta_win
        else :
            self.delta = self.delta_lose

        # 3) update policy and differences
        ...
        for action in self.g.actions(self.playerID, s):
            if action == argmax:
                ...
                self.pi[s][action] = min(self.pi[s][action],1.) #to restrain to probablity
                ...
                self.pi[s][action] = max(0.,self.pi[s][action]) #to restrain to probablity
                self.Delta2[s][action] = D - self.Delta[s][action]
                self.Delta[s][action] = D
    
        # 4) decay et other updates
